...
shapefile = r"F:\Jyoti Shukla -MS\arctry\Rajasthan shapefile\Rajasthan.shp"
output_path = r"F:\Jyoti Shukla -MS\AVHRR Rajasthan data (1981-2022)"
...
VHI_image_train = AVHRR_data_1981_2022[0:2000]
VHI_image_test = AVHRR_data_1981_2022[2000:]

#saving and reading as txt file

vhi_train_file = open(r"F:\Jyoti Shukla -MS\rajasthan dataset/Train_rajasthan_AVHRR_data_1981_2022.txt","w")
for array in VHI_image_train:
  np.savetxt(vhi_train_file,array)
vhi_train_file.close()

vhi_test_file = open(r"F:\Jyoti Shukla -MS\rajasthan dataset/Test_rajasthan_AVHRR_data_1981_2022.txt","w")
for array in VHI_image_test:
  np.savetxt(vhi_test_file,array)
vhi_test_file.close()

#.................loading data..........................................................................#
VHI_train = np.loadtxt(r"F:\Jyoti Shukla -MS\rajasthan dataset/Train_rajasthan_AVHRR_data_1981_2022.txt").reshape(2000,197,243)
VHI_test = np.loadtxt(r"F:\Jyoti Shukla -MS\rajasthan dataset/Test_rajasthan_AVHRR_data_1981_2022.txt").reshape(66,197,243)
...
#=======================================================================================================#
VHI_train_padded_file = open(r"F:\Jyoti Shukla -MS\rajasthan dataset/Padded_Train_rajasthan_AVHRR_data_1981_2022.txt","w")
for i in range(len(VHI_train)):
  VHI_train[i] = np.where(VHI_train[i]<0,0,VHI_train[i]) #removing nodata
  VHI_train_trim = VHI_train[i][5:,:]
  #VHI_train[i+1] = np.where(VHI_train[i+1]<0,0,VHI_train[i+1])
  # pad=VHI_train[i]
  pad = np.pad(VHI_train_trim,((0,0),(6,7))) # padding to get divisible by 2 shape (190,128)
  #pad_label = np.pad(VHI_train[i+1],((2,0),(0,4)))
  np.savetxt(VHI_train_padded_file,pad)
VHI_train_padded_file.close()

VHI_test_padded_file = open(r"F:\Jyoti Shukla -MS\rajasthan dataset/Padded_Test_rajasthan_AVHRR_data_1981_2022.txt","w")
for i in range(len(VHI_test)):
  VHI_test[i] = np.where(VHI_test[i]<0,0,VHI_test[i]) #removing nodata
  VHI_test_trim = VHI_test[i][5:,:]
  #VHI_train[i+1] = np.where(VHI_train[i+1]<0,0,VHI_train[i+1])
  #pad=VHI_test[i]
  pad = np.pad(VHI_test_trim,((0,0),(6,7))) # padding to get divisible by 2 shape (190,128)
  #pad_label = np.pad(VHI_train[i+1],((2,0),(0,4)))
...
for i in range(len(VHI_train)-1):
  ...
  VHI_train_trim = VHI_train[i][5:,:]
  VHI_train[i+1] = np.where(VHI_train[i+1]<0,0,VHI_train[i+1])
  VHI_train_label_trim = VHI_train[i+1][5:,:]
  # pad = VHI_train[i]
  # pad_label = VHI_train[i+1]
  pad = np.pad(VHI_train_trim,((0,0),(6,7))) # padding to get divisible by 2 shape (190,128)
  pad_label = np.pad(VHI_train_label_trim,((0,0),(6,7)))
  #scaling in 0 to 1 range
  scaler = MinMaxScaler()
  scaled = scaler.fit_transform(pad)
  scales.append(scaled)
  scaled_label = scaler.fit_transform(pad_label)
  train_array.append((scaled.reshape(192,256,1))) # giving the next image as label for the current image
  train_labels.append((scaled_label.reshape(192,256,1)))
len(train_labels), train_array[1].shape
plt.imshow(train_array[0].reshape(192,256))
...
for i in range(len(VHI_test)-1):
  ...
  VHI_test_label_trim = VHI_test[i+1][5:,:]
  # pad = VHI_test[i]
  # pad_label= VHI_test[i+1]
  pad = np.pad(VHI_test_trim,((0,0),(6,7))) # padding to get divisible by 2 shape (190,128)
  pad_label = np.pad(VHI_test_label_trim,((0,0),(6,7)))
  #scaling in 0 to 1 range
  scaler = MinMaxScaler()
  scaled = scaler.fit_transform(pad)
  #scales.append(scaled)
  scaled_label = scaler.fit_transform(pad_label)
  test_array.append(scaled.reshape(192,256,1)) # giving the next image as label for the current image
  test_labels.append(scaled_label.reshape(192,256,1))
len(test_labels)
...
x = train_array
y = train_labels
np.savez(r"F:\Jyoti Shukla -MS\rajasthan dataset/paired_arrays_train_rajasthan_AVHRR_data.npz",x = x , y = y)

x = test_array
y = test_labels
np.savez(r"F:\Jyoti Shukla -MS\rajasthan dataset/paired_arrays_test_rajasthan_AVHRR_data.npz",x = x , y = y)
