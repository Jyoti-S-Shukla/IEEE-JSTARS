...
shapefile = r"F:\Jyoti Shukla -MS\Shapefiles for ROI\Peninsular_India.shp"
output_path = r"F:\Jyoti Shukla -MS\AVHRR peninsular india data (1981-2022)"
...
VHI_image_train = AVHRR_data_1981_2022[0:1900]
VHI_image_test = AVHRR_data_1981_2022[1900:1961]

#saving and reading as txt file

vhi_train_file = open(r"F:\Jyoti Shukla -MS\Peninsular dataset/Peninsular_Train_AVHRR_data_1981_2022.txt","w")
for array in VHI_image_train:
  np.savetxt(vhi_train_file,array)
vhi_train_file.close()

vhi_test_file = open(r"F:\Jyoti Shukla -MS\Peninsular dataset/Peninsular_Test_AVHRR_data_1981_2022.txt","w")
for array in VHI_image_test:
  np.savetxt(vhi_test_file,array)
vhi_test_file.close()

#.................loading data..........................................................................#
VHI_train = np.loadtxt(r"F:\Jyoti Shukla -MS\Peninsular dataset/Peninsular_Train_AVHRR_data_1981_2022.txt").reshape(1900,443,575)
VHI_test = np.loadtxt(r"F:\Jyoti Shukla -MS\Peninsular dataset/Peninsular_Test_AVHRR_data_1981_2022.txt").reshape(61,443,575)

np.max(VHI_train), np.min(VHI_train), np.max(VHI_test), np.min(VHI_test)

#.....................................saving the padded array and labels................................#
#=======================================================================================================#
VHI_train_padded_file = open(r"F:\Jyoti Shukla -MS\Peninsular dataset/Padded_Peninsular_Train_AVHRR_data_1981_2022.txt","w")
for i in range(len(VHI_train)):
  ...
  pad = np.pad(VHI_train[i],((5,0),(0,1))) # padding to get divisible by 2 shape (190,128)
  #pad_label = np.pad(VHI_train[i+1],((2,0),(0,4)))
  np.savetxt(VHI_train_padded_file,pad)
VHI_train_padded_file.close()

VHI_test_padded_file = open(r"F:\Jyoti Shukla -MS\Peninsular dataset/Padded_Peninsular_Test_AVHRR_data_1981_2022.txt","w")
...
for i in range(len(VHI_train)-1):
  ...
  VHI_train[i+1] = np.where(VHI_train[i+1]<0,0,VHI_train[i+1])
  pad = np.pad(VHI_train[i],((5,0),(0,1))) # padding to get divisible by 2 shape (190,128)
  pad_label = np.pad(VHI_train[i+1],((5,0),(0,1)))
  #scaling in 0 to 1 range
  scaler = MinMaxScaler()
  scaled = scaler.fit_transform(pad)
  scales.append(scaled)
  scaled_label = scaler.fit_transform(pad_label)
  train_array.append((scaled.reshape(448,576,1))) # giving the next image as label for the current image
  train_labels.append((scaled_label.reshape(448,576,1)))
len(train_labels), train_array[1].shape
plt.imshow(train_array[0].reshape(448,576))
...
for i in range(len(VHI_train)-1):
  ...
  scaler = MinMaxScaler()
  scaled = scaler.fit_transform(pad)
  unscaled = scaler.inverse_transform(train_array[i].reshape(448,576))
...
for i in range(len(VHI_test)-1):
  ...
  pad = np.pad(VHI_test[i],((5,0),(0,1))) # padding to get divisible by 2 shape (190,128)
  pad_label = np.pad(VHI_test[i+1],((5,0),(0,1)))
  #scaling in 0 to 1 range
  scaler = MinMaxScaler()
  scaled = scaler.fit_transform(pad)
  #scales.append(scaled)
  scaled_label = scaler.fit_transform(pad_label)
  test_array.append(scaled.reshape(448,576,1)) # giving the next image as label for the current image
  test_labels.append(scaled_label.reshape(448,576,1))
len(test_labels)
...
x = train_array
y = train_labels
np.savez("F:\Jyoti Shukla -MS\Peninsular dataset/paired_padded_peninsular_arrays_train_AVHRR_data.npz",x = x , y = y)

x = test_array
y = test_labels
np.savez("F:\Jyoti Shukla -MS\Peninsular dataset/paired_padded_peninsular_arrays_test_AVHRR_data.npz",x = x , y = y)
